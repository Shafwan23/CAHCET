...
const TermsEditor = () => {
  ...
  const [form, setForm] = useState({ title: 'Terms & Conditions', lastUpdated: '', content: '' });
  ...
  const fetchPage = async () => {
    try {
      ...
      setSectionsMap(map);
      if (map['about.terms']) setForm(JSON.parse(map['about.terms'].draftContent || map['about.terms'].content || '{}') || { title: 'Terms & Conditions', lastUpdated: '', content: '' });
    } catch (err) { toast({ type: 'error', title: 'Error', message: 'Failed to load Terms.' }); } finally { setLoading(false); }
  };
  useEffect(() => { fetchPage(); }, []);
  ...
  const handleSaveDraft = async (isSilent = false) => {
    ...
    try {
      ...
      if (sectionsMap['about.terms']) {
        await cmsService.updateSection(sectionsMap['about.terms'].id, { draftContent: content, _isSilentDraft: isSilent });
      } else {
        const newSec = await cmsService.createSection({ pageId, sectionKey: 'about.terms', title: 'Terms & Conditions', type: 'json', draftContent: content, _isSilentDraft: isSilent });
  ...
  const { status, lastModified, validationIssues } = useEditorStatus(sectionsMap, 'about.terms', form);

  if (loading && !Object.keys(sectionsMap).length) return <div>Loading...</div>;

  return (
    <EditorPage title="Terms & Conditions" description="Manage the website terms of service and conditions." breadcrumb={['Admin', 'About', 'Terms & Conditions']} onSave={() => handleSaveDraft(false)} onPublish={async () => { await handleSaveDraft(true); setPreviewSection(sectionsMap['about.terms']); }} onReset={fetchPage} isLoading={loading} status={status} lastModified={lastModified} validationIssues={validationIssues}>
        ...
        <div className="xl:col-span-4 hidden xl:block">
           <div className="sticky top-40 bg-white rounded-2xl border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.06)] overflow-hidden">
             ...
             <div className="p-6 prose prose-sm max-w-none text-slate-600">
               <h3 className="text-xl font-bold text-slate-900 mb-1">{form.title || 'Terms & Conditions'}</h3>
