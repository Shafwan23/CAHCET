...
import { Plus, Pencil, Trash2, Search, CheckCircle, ArrowLeft, Image as ImageIcon } from 'lucide-react';
...
const GalleryEditor = () => {
  ...
  const fetchPage = async () => {
    try {
      ...
      setSectionsMap(map);
      if (map['institution.gallery']) setItems(JSON.parse(map['institution.gallery'].draftContent || map['institution.gallery'].content || '[]'));
  ...
  const handleSaveDraft = async (isSilent = false, newItems = null) => {
    ...
    try {
      ...
      if (sectionsMap['institution.gallery']) {
        await cmsService.updateSection(sectionsMap['institution.gallery'].id, { draftContent: content, _isSilentDraft: isSilent });
      } else {
        const newSec = await cmsService.createSection({ pageId, sectionKey: 'institution.gallery', title: 'Gallery', type: 'json', draftContent: content, _isSilentDraft: isSilent });
        setSectionsMap(prev => ({ ...prev, ['institution.gallery']: newSec.data }));
      }
  ...
  const { status, lastModified, validationIssues } = useEditorStatus(sectionsMap, 'institution.gallery', items);
  ...
  return (
    <EditorPage title="Gallery" description="Manage gallery records." breadcrumb={['Admin', 'Institution Records', 'Gallery']} onSave={() => handleSaveDraft(false)} onPublish={async () => { await handleSaveDraft(true); setPreviewSection(sectionsMap['institution.gallery']); }} onReset={fetchPage} isLoading={loading} status={status} lastModified={lastModified} validationIssues={validationIssues}>
