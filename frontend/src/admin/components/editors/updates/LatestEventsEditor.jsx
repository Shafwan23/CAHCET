...
import {
  ...
  MapPin, CheckCircle, Search, Pin, Eye, EyeOff
} from 'lucide-react';
...
const EventCard = ({ item, onEdit, onDelete, onTogglePin, onTogglePublish }) => (
    ...
    <div className="flex gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-2 space-y-1">
          {item.eventDate && (
            <p className="text-xs text-slate-500 flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> {item.eventDate}</p>
          )}
          {item.venue && (
            <p className="text-xs text-slate-500 flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" /> {item.venue}</p>
          )}
        </div>
      </div>
    </div>
...
const EventModal = ({ initial, onSave, onClose }) => {
  const { admin } = useAdminAuth();
  const [form, setForm] = useState(initial?.id ? initial : { ...createEmptyItem(UPDATE_TYPES.EVENTS), author: admin || '' });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-100 sticky top-0 bg-white z-10 shrink-0">
          <h3 className="text-base font-bold text-slate-800">{initial?.id ? 'Edit Event' : 'Create Event'}</h3>
        ...
        <div className="p-5 space-y-5 overflow-y-auto min-h-0 flex-1">
          ...
          <div className="flex gap-4">
            <div className="w-24 h-24 rounded-xl bg-slate-100 shrink-0 overflow-hidden">
              {form.image ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-slate-300"><Calendar className="w-8 h-8" /></div>
              )}
            </div>
            <div>
              ...
              {form.image && <button onClick={() => update('image', '')} className="block mt-2 text-xs text-amber-500 hover:underline">Remove Image</button>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <input className={inputCls} value={form.title} onChange={e => update('title', e.target.value)} placeholder="e.g. Annual Tech Symposium" />
            </div>

            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5">Venue</label>
              <input className={inputCls} value={form.venue} onChange={e => update('venue', e.target.value)} placeholder="e.g. Main Auditorium" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5">Date</label>
              <input className={inputCls} type="date" value={form.eventDate} onChange={e => update('eventDate', e.target.value)} />
            </div>

            <div className="sm:col-span-2">
              <label className="block text-xs font-semibold text-slate-600 mb-1.5">Description</label>
              <textarea className={`${inputCls} resize-none`} rows={4} value={form.description} onChange={e => update('description', e.target.value)} placeholder="Detailed description of the event..." />
            </div>
          </div>
...
export default function LatestEventsEditor() {
  const { admin } = useAdminAuth();
  ...
  const refresh = async () => {
    setLoading(true);
    if (search) setItems(await updatesService.search(UPDATE_TYPES.EVENTS, search));
    else setItems(await updatesService.getAll(UPDATE_TYPES.EVENTS));
    setLoading(false);
  };
  ...
  const handleSave = async (form) => {
    if (form.id) await updatesService.update(UPDATE_TYPES.EVENTS, form.id, form, admin);
    else await updatesService.add(UPDATE_TYPES.EVENTS, form, admin);
    await refresh();
    setEditing(null);
    setSaved(true);
  ...
  const executeDelete = async () => {
    ...
    await updatesService.delete(UPDATE_TYPES.EVENTS, confirmDelete.id);
    await refresh();
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Latest Events</h2>
          <p className="text-sm text-slate-500 mt-1">Manage college events, seminars, and fests.</p>
        </div>
      ...
      <div className="relative max-w-sm">
        ...
        <input className={`${inputCls} pl-10`} placeholder="Search events..." value={search} onChange={e => setSearch(e.target.value)} />
      </div>
          ...
          <AnimatePresence>
            {items.map(item => (
              <EventCard
                ...
                onTogglePin={async (i) => { await updatesService.togglePin(UPDATE_TYPES.EVENTS, i.id, admin); await refresh(); }}
                onTogglePublish={async (i) => { await updatesService.togglePublish(UPDATE_TYPES.EVENTS, i.id, admin); await refresh(); }}
              />
            ))}
          </AnimatePresence>
        </div>
