...
import { Plus, Pencil, Trash2, Search, CheckCircle, ArrowLeft, Users } from 'lucide-react';
...
const FacultyEditor = () => {
  ...
  const fetchPage = async () => {
    try {
      ...
      setSectionsMap(map);
      if (map['institution.faculty']) setItems(JSON.parse(map['institution.faculty'].draftContent || map['institution.faculty'].content || '[]'));
  ...
  const handleSaveDraft = async (isSilent = false, newItems = null) => {
    ...
    try {
      ...
      if (sectionsMap['institution.faculty']) {
        await cmsService.updateSection(sectionsMap['institution.faculty'].id, { draftContent: content, _isSilentDraft: isSilent });
      } else {
        const newSec = await cmsService.createSection({ pageId, sectionKey: 'institution.faculty', title: 'Faculty', type: 'json', draftContent: content, _isSilentDraft: isSilent });
        setSectionsMap(prev => ({ ...prev, ['institution.faculty']: newSec.data }));
      }
  ...
  const { status, lastModified, validationIssues } = useEditorStatus(sectionsMap, 'institution.faculty', items);
  ...
  return (
    <EditorPage title="Faculty" description="Manage faculty records." breadcrumb={['Admin', 'Institution Records', 'Faculty']} onSave={() => handleSaveDraft(false)} onPublish={async () => { await handleSaveDraft(true); setPreviewSection(sectionsMap['institution.faculty']); }} onReset={fetchPage} isLoading={loading} status={status} lastModified={lastModified} validationIssues={validationIssues}>
