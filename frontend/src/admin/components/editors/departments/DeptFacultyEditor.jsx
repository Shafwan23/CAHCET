...
import { Monitor, Upload, Plus, Trash2, ArrowUp, ArrowDown, Users, Mail, UserCircle, Search, X } from 'lucide-react';
...
const DeptFacultyEditor = ({ deptKey, dept, cms, session }) => {
  ...
  useEffect(() => {
    if (cms.data?.faculties) {
      setFaculties(Array.isArray(cms.data.faculties) ? cms.data.faculties : []);
    } else {
      setFaculties([]);
    }
  }, [deptKey, cms.data]);
  ...
  const handleSave = async (isSilent = false) => {
    setLoading(true);
    try {
      cms.setSection('faculties', faculties);
      await cms.saveSection('faculties', session?.username, session?.name, isSilent);
      if (!isSilent) addToast({ type: 'success', title: 'Draft Saved', message: `Faculty changes saved to draft.` });
    } catch(e) {
  ...
  const handlePublishClick = async () => {
    ...
    if (cms.publishSection) {
       await cms.publishSection('faculties');
       addToast({ type: 'success', title: 'Live', message: 'Faculties published to production.' });
    }
  };

  const handleReset = () => {
    const fresh = cms.data?.faculties || [];
    setFaculties(Array.isArray(fresh) ? fresh : []);
    cms.setSection('faculties', fresh);
  ...
  const updateItem = (index, field, value) => {
    ...
    cms.setSection('faculties', updated);
  };
  ...
  const addItem = () => {
    const newItem = { ...emptyFaculty, id: `fac_${Date.now()}` };
    const updated = [newItem, ...faculties]; // add to top so it's visible immediately
    setFaculties(updated);
    cms.setSection('faculties', updated);
    setSearch('');
  };
  ...
  const handleImageUpload = async (e, index) => {
    ...
    try {
      ...
      const rec = await fileService.upload(compressed, deptKey, 'faculty');
  ...
  faculties.forEach((fac, idx) => {
     if (!fac.name?.trim()) validationIssues.push(`Faculty card ${idx + 1} is missing a name.`);
  });

  const filtered = search
    ? faculties.filter(f => f.name.toLowerCase().includes(search.toLowerCase()) || f.designation.toLowerCase().includes(search.toLowerCase()))
    : faculties;

  return (
    <EditorPage
      title="Faculty Editor"
      description="Manage the eminent faculties, profiles, and qualifications."
      breadcrumb={['Admin', 'Departments', dept.abbr, 'Faculty']}
      onSave={() => handleSave(false)}
      onPublish={handlePublishClick}
      onReset={handleReset}
      isLoading={loading}
      status={cms.status?.faculties || 'DRAFT'}
      lastModified={cms.lastModified?.faculties}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        <div className="xl:col-span-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
             ...
             <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-[2rem] border border-slate-700 shadow-[0_10px_40px_rgba(0,0,0,0.15)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.25)] hover:-translate-y-1 transition-all duration-500 flex flex-col justify-between h-36 relative overflow-hidden group text-white">
                <div className="flex justify-between items-start">
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Ph.D Holders</p>
                   <UserCircle className="w-5 h-5 text-emerald-500" />
                </div>
                <div className="absolute -top-10 -right-10 w-32 h-32 bg-indigo-500/20 rounded-full blur-2xl group-hover:bg-indigo-500/40 transition-colors duration-500 pointer-events-none" />
                <div className="relative z-10">
                   <p className="text-5xl font-black text-white tracking-tighter drop-shadow-md">
                      {faculties.filter(f => f.qualification?.toLowerCase().includes('ph.d')).length}
                   </p>
                </div>
             </div>
          </div>
          ...
          <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} className="space-y-4">
            <AnimatePresence>
              {filtered.map((item, index) => {
                const actualIndex = faculties.findIndex(f => f.id === item.id);
                return (
                  <motion.div 
                    layout 
                      ...
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            <span className="text-base font-extrabold text-slate-900 block leading-tight tracking-tight">
                              {item.name || 'New Faculty'}
                            </span>
                            <span className="text-[10px] text-amber-600 font-bold tracking-widest uppercase">{item.designation}</span>
                         </div>
                      </div>
                        ...
                        {!search && (
                           <>
                              ...
                              <button
                                ...
                                disabled={actualIndex === faculties.length - 1}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <select 
                             ...
                             onChange={e => updateItem(actualIndex, 'designation', e.target.value)}
                          >
                        ...
                        <AdminInput
                          ...
                          onChange={e => updateItem(actualIndex, 'qualification', e.target.value)}
                          placeholder="Ph.D., M.Tech"
                        />
                        <AdminInput
                          label="Specialization"
                          value={item.specialization || ''}
                          onChange={e => updateItem(actualIndex, 'specialization', e.target.value)}
                        ...
                        <AdminInput
                          ...
                          type="email"
                          value={item.email || ''}
                          onChange={e => updateItem(actualIndex, 'email', e.target.value)}
                          placeholder="faculty@cahcet.edu.in"
                        />
                        <AdminInput
                          label="Experience (years)"
                          type="number"
                          value={item.experience || ''}
                          onChange={e => updateItem(actualIndex, 'experience', e.target.value)}
                          placeholder="15"
                        />
                      </div>
                      ...
                      <div className="flex items-center gap-4 pt-2 border-t border-slate-50">
                        ...
                        <div className="flex-1">
                           <AdminInput
                             ...
                             onChange={e => updateItem(actualIndex, 'publications', e.target.value)}
                             placeholder="0"
                           />
                        </div>
                      </div>
              ...
              {faculties.length === 0 && !search && (
                <div className="text-center p-12 bg-white rounded-2xl border border-dashed border-slate-300">
                   <Users className="w-12 h-12 text-slate-300 mx-auto mb-3" />
                   <h3 className="text-sm font-bold text-slate-700">No Faculty Added</h3>
                   <p className="text-xs text-slate-500 mt-1 mb-4">Start adding faculty members to build the directory.</p>
                   <button onClick={addItem} className="px-4 py-2 bg-slate-900 text-white text-xs font-bold rounded-lg hover:bg-slate-800 transition-colors">Add Faculty</button>
                </div>
              )}
            </AnimatePresence>
          </motion.div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.06)] overflow-hidden flex-1 flex flex-col">
              <div className="bg-slate-50 border-b border-slate-100 px-4 py-3 flex items-center gap-2 shrink-0">
                ...
                <div className="mx-auto bg-white border border-slate-200 rounded-md px-3 py-1 text-[10px] text-slate-400 font-mono flex-1 max-w-[200px] text-center truncate shadow-sm">
                  cahcet.edu.in/departments/{deptKey}/faculties
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto bg-primary-50/50 p-6 relative">
                 <div className="scale-[0.8] origin-top">
                    <FacultySection data={faculties} departmentName={dept.fullName} />
                 </div>
              </div>
            </div>
          </div>
